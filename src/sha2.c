/* SHA2 support, from apr 1.2.7, for use by mod_auth_tkt. Unfortunately
 ...
 * only.
 */
void mat_SHA512_Last(SHA512_CTX*);
void mat_SHA256_Transform(SHA256_CTX*, const sha2_word32*);
void mat_SHA512_Transform(SHA512_CTX*, const sha2_word64*);
...
/*** SHA-256: *********************************************************/
void mat_SHA256_Init(SHA256_CTX* context) {
...
void mat_SHA256_Transform(SHA256_CTX* context, const sha2_word32* data) {
...
void mat_SHA256_Update(SHA256_CTX* context, const sha2_byte *data, size_t len) {
        ...
        if (usedspace > 0) {
                ...
                if (len >= freespace) {
                        ...
                        len -= freespace;
                        data += freespace;
                        mat_SHA256_Transform(context, (sha2_word32*)context->buffer);
                } else {
        ...
        while (len >= SHA256_BLOCK_LENGTH) {
                ...
                mat_SHA256_Transform(context, (sha2_word32*)data);
...
void mat_SHA256_Final(sha2_byte digest[], SHA256_CTX* context) {
...
char* mat_SHA256_Data(const sha2_byte* data, size_t len, char digest[SHA256_DIGEST_STRING_LENGTH]) {
        SHA256_CTX      context;

        mat_SHA256_Init(&context);
        mat_SHA256_Update(&context, data, len);
        return mat_SHA256_End(&context, digest);
}


/*** SHA-512: *********************************************************/
void mat_SHA512_Init(SHA512_CTX* context) {
...
void mat_SHA512_Transform(SHA512_CTX* context, const sha2_word64* data) {
...
void mat_SHA512_Update(SHA512_CTX* context, const sha2_byte *data, size_t len) {
        unsigned int    freespace, usedspace;
        ...
        if (usedspace > 0) {
                ...
                if (len >= freespace) {
                        ...
                        len -= freespace;
                        data += freespace;
                        mat_SHA512_Transform(context, (sha2_word64*)context->buffer);
                } else {
                        /* The buffer is not yet full */
                        MEMCPY_BCOPY(&context->buffer[usedspace], data, len);
        ...
        while (len >= SHA512_BLOCK_LENGTH) {
                /* Process as many complete blocks as we can */
                mat_SHA512_Transform(context, (sha2_word64*)data);
...
void mat_SHA512_Last(SHA512_CTX* context) {
        unsigned int    usedspace;
...
void mat_SHA512_Final(sha2_byte digest[], SHA512_CTX* context) {
...
char* mat_SHA512_Data(const sha2_byte* data, size_t len, char digest[SHA512_DIGEST_STRING_LENGTH]) {
        SHA512_CTX      context;

        mat_SHA512_Init(&context);
        mat_SHA512_Update(&context, data, len);
        return mat_SHA512_End(&context, digest);
}


/*** SHA-384: *********************************************************/
void mat_SHA384_Init(SHA384_CTX* context) {
...
void mat_SHA384_Update(SHA384_CTX* context, const sha2_byte* data, size_t len) {
        mat_SHA512_Update((SHA512_CTX*)context, data, len);
}

void mat_SHA384_Final(sha2_byte digest[], SHA384_CTX* context) {
        sha2_word64     *d = (sha2_word64*)digest;
        ...
        /* If no digest buffer is passed, we don't bother doing this: */
        if (digest != (sha2_byte*)0) {
                mat_SHA512_Last((SHA512_CTX*)context);
...
char* mat_SHA384_Data(const sha2_byte* data, size_t len, char digest[SHA384_DIGEST_STRING_LENGTH]) {
        SHA384_CTX      context;

        mat_SHA384_Init(&context);
        mat_SHA384_Update(&context, data, len);
        return mat_SHA384_End(&context, digest);
}
